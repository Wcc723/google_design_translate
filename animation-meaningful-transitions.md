...
- **共用的元素：**從漸變動畫開始到結束都持續存在的元素，可能是細小的icon圖示或顯眼的大型圖片，透過動畫改變成符合螢幕的尺寸。
...
- 思考使用者的注意力應該如何被引導？什麼樣的元素或動作能夠協助這個目標？動畫過程中，要進入、離開或共用的元素，要如何安排強調或弱化？
- 設計畫面時，思考動畫前後的狀態，在漸變的過程中透過顏色及共用的元素，找到機會創造視覺關聯性。
...
> 你該這麼做：
>
> 圖層式的紙張元素是常用的設計，紙張元素常常以滑入的方式進入畫面，而墨水元素則較常使用淡入的方式。不建議使用全螢幕的淡出效果，但是至少比直接切換畫面好。
...
當你建置一個動畫，要考量該元素的移動方式與移動的時間差。確保動畫能幫助你更層次化的顯示資訊；即透過視線動向的安排來傳達給使用者，什麼是最重要的資訊。
...
- 避免線性空間路徑，除非運動方式是沿著特定軸線，或與其他元素一起朝向或離開特定目標。
- 確保各個元素的路徑相互配合。避免有衝突或重疊的路徑。
- 考慮動畫的故事性：什麼元素在什麼元素下移動，以及為何如此？
- 如果在螢幕上追蹤所有移動元素的路徑，看起來是否漂亮且具組織性？這些路徑是否有創造出清楚的畫面引導使用者該注意哪個位置？
- 在進入與離開的元素上套用一致的動畫，強化各元素的空間關係。
